...
async function problemB() {
  ...
  console.log("soy promises", promises);
...
async function problemC() {
  ...
  // AsyncAwait version
  let promesas = filenames.map((f) => promisifiedReadFile(f));
  await Promise.all(promesas).then((stanzas) => {
...
async function problemD() {
  ...
  try {
    ...
    /*   filenames.map(elem => {
      blue(await promisifiedReadFile(elem))
    }) */
  } catch (error) {
    magenta(error);
  } finally {
